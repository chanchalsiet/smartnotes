...
function editNote(noteId) {
    ...
    fetch(`http://localhost:8000/edit_notes/${noteId}`, {
        method: "POST",
        headers: {
...
function deleteNote(noteId) {
    ...
    const token = localStorage.getItem("token");
    fetch(`http://localhost:8000/delete_notes/${noteId}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
...
addNoteBtn.addEventListener("click", async (e) => {
    ...
    const res = await fetch("http://localhost:8000/add_notes", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
...
async function loadNotes() {
    const token = localStorage.getItem("token");
    const res = await fetch("http://localhost:8000/all_notes/", {
        headers: {
            "Authorization": `Bearer ${token}`
        }
    });
